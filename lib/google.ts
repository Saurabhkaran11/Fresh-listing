...
type UserSettings = { spreadsheet_id: string | null; google_refresh_token: string | null; google_access_token: string | null; google_access_expires_at: number | null };
...
export function oauthUrl(origin: string, state: string) {
  ...
  url.search = new URLSearchParams({ client_id: config.clientId, redirect_uri: `${origin}/api/google/oauth/callback`, response_type: "code", access_type: "offline", prompt: "consent", include_granted_scopes: "true", scope: "https://www.googleapis.com/auth/drive.file https://www.googleapis.com/auth/spreadsheets", state }).toString();
  return url.toString();
}

export async function getSettings(database: D1Database, userId: string) {
  const result = await database.prepare("SELECT spreadsheet_id, google_refresh_token, google_access_token, google_access_expires_at FROM user_settings WHERE owner_user_id = ?").bind(userId).first<UserSettings>();
  return result || { spreadsheet_id: null, google_refresh_token: null, google_access_token: null, google_access_expires_at: null };
}
