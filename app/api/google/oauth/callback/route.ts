...
export async function GET(request: Request) {
  ...
  try {
    ...
    const googleAccountEmail = String(profile.email || "").trim().toLowerCase() || null;
    ...
    await database.prepare("UPDATE user_settings SET google_account_email = ?, google_refresh_token = ?, google_access_token = ?, google_access_expires_at = ?, updated_at = CURRENT_TIMESTAMP WHERE owner_user_id = ?").bind(googleAccountEmail, refreshToken, await encryptToken(tokens.access_token), Date.now() + (tokens.expires_in || 3600) * 1000, user.userId).run();
