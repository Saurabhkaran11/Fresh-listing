...
export async function GET() {
  ...
  try {
    ...
    return Response.json({ connected: Boolean(settings.google_refresh_token), googleAccountEmail: settings.google_account_email, spreadsheetUrl: settings.spreadsheet_id ? `https://docs.google.com/spreadsheets/d/${settings.spreadsheet_id}/edit` : null, driveFolderUrl: settings.google_drive_folder_id ? `https://drive.google.com/drive/folders/${settings.google_drive_folder_id}` : null, googleConfigured: Boolean(config.GOOGLE_CLIENT_ID && config.GOOGLE_CLIENT_SECRET && config.GOOGLE_TOKEN_ENCRYPTION_KEY), providerConfigured: Boolean(config.SEARCHAPI_API_KEY), emailConfigured: Boolean(config.RESEND_API_KEY && config.EMAIL_FROM) });
