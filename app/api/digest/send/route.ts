...
export async function POST() {
  ...
  try {
    ...
    const rows = await database.prepare("SELECT title, company, source_portal, provider, source, location, direct_url, posted_at FROM job_postings WHERE owner_user_id = ? ORDER BY created_at DESC LIMIT 50").bind(user.userId).all<{ title: string; company: string; source_portal: string; provider: string; source: string; location: string; direct_url: string; posted_at: string | null }>();
    const jobs = rows.results || [];
    if (!jobs.length) return Response.json({ error: "Run a scrape before sending a digest." }, { status: 400 });
    const html = `<h2>Fresh Listings — ${jobs.length} opportunities</h2><p>Here are your latest saved matches.</p><ul>${jobs.map((job) => `<li><a href="${escapeHtml(job.direct_url)}"><strong>${escapeHtml(job.title)}</strong></a> — ${escapeHtml(job.company)} · Portal: ${escapeHtml(job.source_portal)} · Provider: ${escapeHtml(job.provider)} · ${escapeHtml(job.location)}${job.posted_at ? ` · ${escapeHtml(job.posted_at)}` : ""}</li>`).join("")}</ul>`;
